...
from optimize_mask_cifar import save_mask_scores
import models
...
parser.add_argument('--checkpoint', type=str, default='./anp/bd.pth', help='The checkpoint to be pruned')
...
parser.add_argument('--pruning-max', type=float, default=0.50, help='the maximum number/threshold for pruning')
...
def main():
    ...
    cl_loss, cl_acc = test(model=model, criterion=criterion, data_loader=clean_test_loader)
    po_loss, po_acc = test(model=model, criterion=criterion, data_loader=poison_test_loader)
    ...
    if args.pruning_by == 'threshold':
        results = evaluate_by_threshold(
            model, mask_values, pruning_max=args.pruning_max, pruning_step=args.pruning_step,
