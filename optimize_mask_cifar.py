...
parser.add_argument('--nb-iter', type=int, default=3000, help='the number of iterations for training')
...
parser.add_argument('--device', type=str, default='cuda:0', help='Device to use for training and testing')
...
#     logger.info('----------- Load Dataset {} with Attack {} --------------'.format(args.dataset, args.attack))
    ...
    save_mask_scores(net, 'mask_values.pth')
...
def save_mask_scores(model, filename="mask_scores.pth"):
    ...
    for key, value in model.state_dict().items():
        if 'mask' in key:
            mask_dict[key] = value
    torch.save(mask_dict, './save/mask_scores.pth')
    return mask_dict
